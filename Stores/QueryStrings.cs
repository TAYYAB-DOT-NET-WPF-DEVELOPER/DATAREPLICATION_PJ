...
namespace DataIntegration.Stores
{
    public class QueryStrings
    {
        ...
        public string POSHDELIVERY = @"SELECT Transact, EmpNum,    MemCode,    OpenDate,    TimeOut,    TimeIn,    PunchIndex,    DeliveryStatus,    RejectedReason,  UpdateStatus,    SNum,    PLink,    TripId,    QLink,    Delivered,    CommissionAmt,PromptConfirmed FROM dba.PosHDelivery where transact in (select transact from dba.posheader where status=3) ";
...
FROM 
    DBA.POSHEADER PH 
JOIN 
     DBA.HOWPAID HP ON PH.TRANSACT = HP.TRANSACT
WHERE 
    PH.TRANSACT IS NOT NULL 
    AND PH.STATUS = 3";
